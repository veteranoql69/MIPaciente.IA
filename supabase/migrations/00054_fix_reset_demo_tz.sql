-- ============================================================
-- Fix: timezone en reset_demo_staging()
-- El error: `v_hoy::timestamptz AT TIME ZONE 'America/Santiago'`
-- retorna un timestamp WITHOUT tz que luego se interpreta como UTC,
-- desplazando todas las citas 4 horas hacia atrás.
-- La forma correcta es timezone(zone, timestamp) que convierte
-- la hora LOCAL a UTC (sentido inverso).
-- ============================================================
...
BEGIN
  ...
  -- ✅ Correcto: interpreta v_hoy como medianoche LOCAL Santiago → convierte a UTC
  -- timezone(zone, timestamp) trata el timestamp como hora local en esa zona
  -- Resultado: 2026-05-11T04:00:00+00 (medianoche Santiago en UTC)
